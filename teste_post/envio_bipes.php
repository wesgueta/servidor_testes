...
if ($conn->connect_error) {
 ...
 file_put_contents("teste_bipes_antes.txt",$responseBody);

 $responseBody2 = str_replace('\\', '', $responseBody);
 $responseBody3 = str_replace('"{', '{', $responseBody2);
 $responseBody4 = str_replace('}"', '}', $responseBody3);
 $responseBody5 = str_replace('(', '[', $responseBody4);
 $responseBody6 = str_replace(')', ']', $responseBody5);

 file_put_contents("teste_bipes_depois.txt",$responseBody6);
 ...
 if(isJSON($responseBody))
 {
  ...
  if(!isset($jsonarr['equipe']) || !isset($jsonarr['bateria']) || !isset($jsonarr['pressao'])  || !isset($jsonarr['temperatura'])  || !isset($jsonarr['giroscopio'])  || !isset($jsonarr['acelerometro'])  || !isset($jsonarr['payload']) )
  {
    ...
    $statuse = "Erro: O JSON recebido nao segue a formatacao correta";
  ...
  //fclose($fp)
    ...
    {
      echo json_encode("Tamanho limite do Payload excedido!");
      $statuse = "Erro: Tamanho limite do Payload excedido";
      $stmt = $conn->prepare("INSERT INTO server_test (statuse, datetimea) VALUES (?, ?)");
      $stmt->bind_param("ss", $statuse, $datetime);
      $stmt->execute();
      $stmt->close();
  ...
  $statuse = "Erro: A requisicao recebida não e um JSON";
  $stmt = $conn->prepare("INSERT INTO server_test (statuse, datetimea) VALUES (?, ?)");
  $stmt->bind_param("ss", $statuse, $datetime);
  $stmt->execute();
 }
